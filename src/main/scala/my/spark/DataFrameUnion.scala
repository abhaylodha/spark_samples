...
object DataFrameUnion extends SparkRunner {
  ...
  def evaluate(
    ...
    val startTime = System.currentTimeMillis
    val d3 = (1 to 23).foldLeft(d1)((df, i) =>
      df.transform(unionOp(_, d2)))
      .repartition(d1.rdd.partitions.size)
    //d3.show(false)
    info("DF1 Partition Size : " + d1.rdd.partitions.size)
    info("DF2 Partition Size : " + d2.rdd.partitions.size)
    info("DF Partition Size : " + d3.rdd.partitions.size)
    info("Count : " + d3.count)
    val endTime = System.currentTimeMillis
    val duration = endTime - startTime
